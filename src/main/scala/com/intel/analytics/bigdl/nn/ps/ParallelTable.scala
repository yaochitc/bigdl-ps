...
(implicit ev: TensorNumeric[T]) extends com.intel.analytics.bigdl.nn.ParallelTable with ParameterSupport[T] {
...
object ParallelTable {
  def apply[@specialized(Float, Double) T: ClassTag]()(implicit ev: TensorNumeric[T]): ParallelTable[T] = {
    new ParallelTable[T]()
  }
}
